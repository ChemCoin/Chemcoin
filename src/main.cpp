...
unsigned int nStakeMinAgeAdjusted = 1 * 60 * 60;    // 4 Hours after block 10000
unsigned int nStakeMaxAge       = 0; // 30 days
...
const string strMessageMagic = "Chemcoin Signed Message:\n";
...
int64_t CTransaction::GetMinFee(unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    if (nBlockSize != 1 && nNewBlockSize >= MAX_BLOCK_SIZE_CHEMX/2)
    {
        if (nNewBlockSize >= MAX_BLOCK_SIZE_CHEMX)
            return MAX_MONEY;
        nMinFee *= MAX_BLOCK_SIZE_CHEMX / (MAX_BLOCK_SIZE_CHEMX - nNewBlockSize);
    }
...
{
    int64_t nSubsidy = 0;
    int64_t nNetworkWeight_ = GetPoSKernelPS() /COIN;
    int64_t nInterestRate = (1 + (17*(nNetworkWeight_ / 1500000)))* CENT;

         if (nHeight <= 1000)
    {
            nInterestRate = (100*(nNetworkWeight_ / nHeight)) * CENT;
            nSubsidy = (nCoinAge * (nInterestRate) * 33 / (365 * 33 + 8));
    }
    else if (nHeight <= 2000)
    {
             nInterestRate = (25*(nNetworkWeight_ / nHeight)) * CENT;
             nSubsidy = (nCoinAge * (nInterestRate) * 33 / (365 * 33 + 8));
    }
    else if (nHeight <= 3000)
    {
             nInterestRate = (12.5*(nNetworkWeight_ / nHeight)) * CENT;
             nSubsidy = (nCoinAge * (nInterestRate) * 33 / (365 * 33 + 8));
    }
    else if (nHeight <= 4000)
    {
             nInterestRate = (6*(nNetworkWeight_ / nHeight)) * CENT;
             nSubsidy = (nCoinAge * (nInterestRate) * 33 / (365 * 33 + 8));
    }
    else if (nHeight <= 5000)
    {
             nInterestRate = (3*(nNetworkWeight_ / nHeight)) * CENT;
             nSubsidy = (nCoinAge * (nInterestRate) * 33 / (365 * 33 + 8));
    }
    else
    {
        nInterestRate = (1 + (17*(nNetworkWeight_ / 1500000)))* CENT;
        nSubsidy = (nCoinAge * (nInterestRate) * 33 / (365 * 33 + 8));
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Chemcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1466635614;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1805295;
...
*/
        ...
        assert(block.hashMerkleRoot == uint256("0x20994710f80c4611a99cbc8378c1a3d25842ec479398e5dd444b7a0455152577"));
        block.print();
